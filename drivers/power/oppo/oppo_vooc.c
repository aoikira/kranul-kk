		...
		(struct oppo_vooc_chip *chip, int vbat_temp_cur)
{
	...
	switch (chip->fastchg_batt_temp_status) {
		case BAT_TEMP_NATURAL:
			if (vbat_temp_cur > chip->vooc_strategy1_batt_up_temp3) {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH0;
				ret = chip->vooc_strategy1_high0_current;
			} else {
			...
			break;
		case BAT_TEMP_HIGH0:
			if (vbat_temp_cur > chip->vooc_strategy1_batt_up_temp4) {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH1;
				ret = chip->vooc_strategy1_high1_current;
			} else {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH0;
				ret = chip->vooc_strategy1_high0_current;
			}
			break;
		case BAT_TEMP_HIGH1:
			if (vbat_temp_cur > chip->vooc_strategy1_batt_up_temp5) {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH2;
				ret = chip->vooc_strategy1_high2_current;
			} else if (vbat_temp_cur < chip->vooc_strategy1_batt_up_down_temp1) {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH0;
				ret = chip->vooc_strategy1_high0_current;
			} else {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH1;
			...
			break;
		case BAT_TEMP_HIGH2:
			if (vbat_temp_cur < chip->vooc_strategy1_batt_up_down_temp2) {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH1;
				ret = chip->vooc_strategy1_high1_current;;
			} else {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH2;
